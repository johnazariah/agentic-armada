...
`ARMADA_ControlPlane__Postgres__ConnectionString`. The `ARMADA_` environment
namespace maps to the root configuration sections, so that variable binds
`ControlPlane:Postgres:ConnectionString`. The connection must point to a
loopback PostgreSQL instance. Configure an absolute path to a local,
...
configuration, including raw `ASPNETCORE_` and `DOTNET_` aliases, then
configures only its validated loopback listener. It never runs migrations
