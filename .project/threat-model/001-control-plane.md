...
configuration sections while the raw environment source remains available to
...
| Environment namespace correction hides a hosting override | prefix-scoped `ARMADA_` configuration binds documented application variables, while raw environment configuration remains visible to reject all endpoint, URL, port, and hosting-preference forms |
